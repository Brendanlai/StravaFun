...
def outputPathHandling(outputFolder):
    if not os.path.exists(outputFolder):
        os.mkdir(outputFolder)
    
    if not os.path.exists(outputFolder + "/laps"):
        os.mkdir(outputFolder + "/laps")
        
    if not os.path.exists(outputFolder + "/points"):
        os.mkdir(outputFolder + "/points")


if __name__ == '__main__':

    folderPath = "/Users/brendanlai/Documents/git/Analysis_practice/strava/23_10_2023_download/fitFiles" # Need to change this accordingly to run properly

    # See if full path was input if not append it
    if os.getcwd() not in folderPath:
        folderPath = os.getcwd() + str(folderPath)

    # Verify path exists 
    if not os.path.exists(folderPath):
        print(f"The folder '{folderPath}' does not exist.")
    else:
        outputFolder = "/Users/brendanlai/Documents/git/Analysis_practice/strava/23_10_2023_download/fit_csvs"
        outputPathHandling(outputFolder) # ensures paths exists, if not creates them

        # Loop for reading, converting, and writing fit files to csvs
        for file in os.listdir(folderPath):
            # ensure file is a fit file
            if str(file[-3:]) != "fit":
                continue

            fname = os.path.join(folderPath, file)
            laps_df, points_df = get_dataframes(fname) # Convert fit files to df

            # Modify fnames to be written to according output folders
            fname_laps = fname.split('.')[0].replace('fitFiles', 'fit_csvs/laps') 
            fname_pts = fname.split('.')[0].replace('fitFiles', 'fit_csvs/points')

            # Write dfs to csvs
            laps_df.to_csv(fname_laps + "_laps_data.csv", index=False) 
            points_df.to_csv(fname_pts + "_points__data.csv", index=False)


    print("Wrote new csv files for all the activities")
